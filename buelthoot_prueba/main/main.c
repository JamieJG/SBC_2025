...
// Declaración global del temporizador
static TimerHandle_t read_timer;
...
static void event_handler(void* arg, esp_event_base_t event_base,
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        if (s_retry_num < MAXIMUM_RETRY) {
            esp_wifi_connect();
            s_retry_num++;
            ESP_LOGI(TAG_WIFI, "Retrying to connect to the WiFi...");
        } else {
            xEventGroupSetBits(s_wifi_event_group, WIFI_FAIL_BIT);
        }
        ESP_LOGI(TAG_WIFI, "Failed to connect to WiFi");
...
static void wifi_init(void) {
    ...
    if (bits & WIFI_CONNECTED_BIT) {
        ...
        ESP_LOGI(TAG_WIFI, "Failed to connect to AP: %s", WIFI_SSID);
    } else {
        ...
        case ESP_GATTC_READ_CHAR_EVT:
            if (p_data->read.status == ESP_GATT_OK) {
                ...
                if (msg_id >= 0) {
                    ...
                    ESP_LOGE(TAG, "Failed to send telemetry to ThingsBoard");
                }
            } else {
                ESP_LOGE(GATTC_TAG, "Failed to read characteristic, status: %d", p_data->read.status);
            }
            break;
...
void read_battery_level(TimerHandle_t xTimer) {
   ...
   if (err != ESP_OK) {
       ESP_LOGE(GATTC_TAG, "Failed to read Battery Level: %s", esp_err_to_name(err));
   }
}
void app_main(void)
{
